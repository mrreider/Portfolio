...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path source) {
            ...
            for (int i = 0; i < source.dataSequence.size(); ++i){
                this.dataSequence.add(source.dataSequence.get(i)); //copy everything over
            }
        ...
        public void extend(Edge edge) {
            this.end.edgesLeaving.add(edge);
    ...
    protected Path dijkstrasShortestPath(T start, T end) {
        if (!vertices.containsKey(start) || !vertices.containsKey(end)) throw new NoSuchElementException("No Path from start to end");
        PriorityQueue<Path> pq = new PriorityQueue<>();
        pq.add(new Path(vertices.get(start))); //add starting vertex to path
        Hashtable<T, Vertex> visited = new Hashtable<>(vertices.size()); //each node will be indexed by its vertex

        while (!pq.isEmpty()){
            Path c = pq.remove(); //remove minimum vertex and store as c
            if (c.end.data == end) return c; //if the end of c is the end node, return that path as it is the minimum.
            int size = vertices.get(c.end.data).edgesLeaving.size();
            if (visited.containsKey(c.end.data)) continue;
            for (int i = 0; i < size; ++i){ // if not the end, go through add copied paths of all edges of current to the priority queue
                Path toAdd = new Path(c);
                toAdd.extend(vertices.get(c.end.data).edgesLeaving.get(i));
                pq.add(toAdd);
            }
            visited.put(c.end.data,c.end);
        }
